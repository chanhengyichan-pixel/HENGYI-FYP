#!/usr/bin/env python3
"""
YOLOv8s training — remaining 4 construction safety datasets.

  EXP 02 — CHVG    : ~/FYP/Dataset/CHVG-Dataset/           (flat VOC XML)
  EXP 03 — SHEL5K  : ~/FYP/Dataset/SHEL5K/...              (VOC XML, Annotations/ + Images/)
  EXP 04 — SH17    : ~/FYP/Dataset/SH17dataset/             (YOLO format, txt split files)
  EXP 05 — SFCHD   : ~/FYP/Dataset/SFCHD-SCALE/dataset_SFCHD/ (YOLO format, broken paths)

Usage:
  python3 train_yolov8_remaining.py --datasets chvg shel5k
  python3 train_yolov8_remaining.py --datasets sh17 sfchd
  python3 train_yolov8_remaining.py --datasets all
"""
...
# ── Constants ──────────────────────────────────────────────────────────────────
HOME         = Path.home()
WEIGHTS      = HOME / "FYP/results/yolov8s.pt"
RESULTS_BASE = HOME / "FYP/results"
SEED         = 42
EPOCHS       = 100
PATIENCE     = 20
BATCH        = 8
...
EXPERIMENTS = {
    "chvg":   {"exp_num": "02", "name": "CHVG"},
    "shel5k": {"exp_num": "03", "name": "SHEL5K"},
    "sh17":   {"exp_num": "04", "name": "SH17"},
    "sfchd":  {"exp_num": "05", "name": "SFCHD"},
}

# Class definitions (discovered by inspecting each dataset)
CLASSES = {
    "chvg": [
        "person", "vest", "white", "yellow", "head", "blue", "glass", "red",
    ],
    "shel5k": [
        "helmet", "head_with_helmet", "person_with_helmet", "face",
    ...
    "sfchd": [
        "person", "helmet", "self_clothes", "safety_clothes",
...
def voc_to_yolo(xml_path: Path, class_map: dict, out_txt: Path) -> bool:
    """Parse one Pascal VOC XML and write a YOLO-format label file."""
    try:
...
def prepare_chvg() -> str:
    """
    CHVG: 1699 image+XML pairs in a flat folder.
    Convert VOC → YOLO, split 70/15/15, copy into CHVG-YOLO/.
    """
    ...
    if yaml_path.exists():
        print("  [CHVG] Already converted — skipping conversion.")
        return str(yaml_path)

    print("  [CHVG] Converting VOC → YOLO ...")
    pairs = []
    ...
    for split, split_pairs in [("train", train_p), ("val", val_p), ("test", test_p)]:
        ...
        for img_path, xml_path in split_pairs:
            ...
            if voc_to_yolo(xml_path, class_map, lbl_dir / (img_path.stem + ".txt")):
                ok += 1
        print(f"    {split}: {ok}/{len(split_pairs)} converted")
    ...
    print(f"  [CHVG] data.yaml written → {yaml_path}")
    return str(yaml_path)


def prepare_shel5k() -> str:
    """
    SHEL5K: 5000 images in Images/, 5000 XMLs in Annotations/.
    Convert VOC → YOLO, split 70/15/15, copy into SHEL5K-YOLO/.
    """
    ...
    if yaml_path.exists():
        print("  [SHEL5K] Already converted — skipping conversion.")
        return str(yaml_path)

    print("  [SHEL5K] Converting VOC → YOLO ...")
    ...
    for split, split_pairs in [("train", train_p), ("val", val_p), ("test", test_p)]:
        ...
        for img_path, xml_path in split_pairs:
            ...
            if voc_to_yolo(xml_path, class_map, sp_lbl / (img_path.stem + ".txt")):
                ok += 1
        print(f"    {split}: {ok}/{len(split_pairs)} converted")
    ...
    print(f"  [SHEL5K] data.yaml written → {yaml_path}")
    return str(yaml_path)


def prepare_sh17() -> str:
    """
    SH17: YOLO format. train_files.txt / val_files.txt contain bare filenames.
    Carve 20% of val → test. Write absolute-path txt files.
    Labels are in archive/labels/ (sibling of archive/images/).
    """
    ...
    if yaml_path.exists():
        print("  [SH17] Already prepared — skipping.")
        return str(yaml_path)

    print("  [SH17] Building absolute-path split files ...")
    img_dir = src / "images"

    train_names = (src / "train_files.txt").read_text().strip().splitlines()
    val_names   = (src / "val_files.txt").read_text().strip().splitlines()

    # Carve 20% of val as test (deterministic)
    ...
    def write_abs(names: list, dest: Path) -> int:
    ...
    write_yaml(yaml_path, {
        "path":  str(src),          # labels/ is a sibling of images/ here
    ...
    print(f"  [SH17] data.yaml written → {yaml_path}")
    return str(yaml_path)


def prepare_sfchd() -> str:
    """
    SFCHD: YOLO format. new_split_yolo/train.txt has hardcoded /home/yfs/ paths
    (broken). Rebuild a 70/15/15 split from the 12 380 local images.
    Labels are in labels/ (sibling of images/).
    """
    ...
    if yaml_path.exists():
        print("  [SFCHD] Already prepared — skipping.")
        return str(yaml_path)

    print("  [SFCHD] Rebuilding 70/15/15 split from local images ...")
    img_dir = src / "images"
    lbl_dir = src / "labels"
    ...
    write_yaml(yaml_path, {
        ...
        "val":   str(out / "val.txt"),
        "test":  str(out / "test.txt"),
        "nc":    len(CLASSES["sfchd"]),
        "names": CLASSES["sfchd"],
    })
    print(f"  [SFCHD] data.yaml written → {yaml_path}")
    return str(yaml_path)


PREPARE_FNS = {
    "chvg":   prepare_chvg,
    "shel5k": prepare_shel5k,
    "sh17":   prepare_sh17,
    "sfchd":  prepare_sfchd,
}

# Paths where test images live after preparation
TEST_IMG_SOURCES = {
...
def get_test_images(key: str, n: int = 100) -> list[Path]:
    ...
    if kind == "dir":
        ...
        imgs = [Path(l.strip()) for l in lines if l.strip()]
...
def train_experiment(key: str, yaml_path: str) -> dict:
    ...
    out_name = f"exp{exp_num}_yolov8_{exp_name}"
    out_dir  = RESULTS_BASE / out_name
    classes  = CLASSES[key]

    print("\n" + "=" * 70)
    print(f"  EXP {exp_num} — YOLOv8s | {exp['name']}")
    print("=" * 70)
    ...
    lines = [
        "=" * 70,
        f"  EXPERIMENT {exp_num} — YOLOv8s on {exp['name']}",
        "=" * 70,
        "",
        "  SETUP",
        f"    Model            : YOLOv8s ({WEIGHTS.name})",
        f"    Dataset          : {exp['name']}",
...
def print_summary(results: list[dict]) -> None:
    ...
    print(sep)
    hdr = (f"  {'Dataset':<10} {'Epochs':>7} {'mAP@0.5':>9} {'Precision':>11}"
           f" {'Recall':>8} {'F1':>7} {'FPS':>7} {'Latency(ms)':>13}")
    print(hdr)
    print(f"  {'-' * 91}")
    for r in results:
        print(
            f"  {r['name']:<10} {r['epochs']:>7d} {r['map50'] * 100:>8.2f}%"
            f" {r['prec'] * 100:>10.2f}% {r['rec'] * 100:>7.2f}%"
            f" {r['f1']:>7.4f} {r['fps']:>7.1f} {r['lat_ms']:>12.2f}"
        )
    print(sep)

    summary_path = RESULTS_BASE / "remaining_experiments_summary.txt"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="YOLOv8s on 4 remaining construction safety datasets",
...
Examples:
  ...
  python3 train_yolov8_remaining.py --datasets all
    ...
    parser.add_argument(
        ...
        choices=["chvg", "shel5k", "sh17", "sfchd", "all"],
        metavar="DATASET",
        help="Which datasets to run (choices: chvg shel5k sh17 sfchd all)",
    )
    ...
    print("=" * 70)
    print("  YOLOv8s — Remaining Experiments")
    print("=" * 70)
    ...
    for key in keys:
        try:
            print(f"\n{'─' * 70}")
            print(f"  Preparing: {EXPERIMENTS[key]['name']} ...")
            yaml_path = PREPARE_FNS[key]()
