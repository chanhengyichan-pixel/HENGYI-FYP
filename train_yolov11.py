#!/usr/bin/env python3
"""
YOLO11s training on all 5 construction safety datasets.

  EXP 11 — Roboflow  : ~/FYP/Dataset/Roboflow Dataset/construction safety.yolov8/
  EXP 12 — CHVG      : reuses ~/FYP/Dataset/CHVG-YOLO/   (converted by YOLOv8 run)
  EXP 13 — SHEL5K    : reuses ~/FYP/Dataset/SHEL5K-YOLO/ (converted by YOLOv8 run)
  EXP 14 — SH17      : reuses ~/FYP/Dataset/SH17-YOLO/   (prepared by YOLOv8 run)
  EXP 15 — SFCHD     : reuses ~/FYP/Dataset/SFCHD-YOLO/  (prepared by YOLOv8 run)

Usage:
  python3 train_yolov11_all.py --datasets all
  python3 train_yolov11_all.py --datasets roboflow chvg shel5k
  python3 train_yolov11_all.py --datasets sh17 sfchd
"""
...
# ── Constants ──────────────────────────────────────────────────────────────────
HOME         = Path.home()
WEIGHTS      = HOME / "FYP/results/weights/yolo11s.pt"
RESULTS_BASE = HOME / "FYP/results/YOLOv11"
SEED         = 42
EPOCHS       = 100
PATIENCE     = 20
BATCH        = 8
...
EXPERIMENTS = {
    "roboflow": {"exp_num": "11", "name": "Roboflow"},
    "chvg":     {"exp_num": "12", "name": "CHVG"},
    "shel5k":   {"exp_num": "13", "name": "SHEL5K"},
    "sh17":     {"exp_num": "14", "name": "SH17"},
    "sfchd":    {"exp_num": "15", "name": "SFCHD"},
}
...
# ── Dataset resolution ─────────────────────────────────────────────────────────
# Reuses YOLO-format datasets already created by the YOLOv8/YOLOv9 runs.
# Falls back to reconverting if missing.
...
def resolve_chvg() -> str:
    ...
    for split, split_pairs in [("train", train_p), ("val", val_p), ("test", test_p)]:
        ...
        for img_path, xml_path in split_pairs:
            ...
            voc_to_yolo(xml_path, class_map, lbl_dir / (img_path.stem + ".txt"))
...
def train_experiment(key: str, yaml_path: str) -> dict:
    ...
    out_name = f"exp{exp_num}_yolov11_{exp_name}"
    out_dir  = RESULTS_BASE / out_name
    classes  = CLASSES[key]

    print("\n" + "=" * 70)
    print(f"  EXP {exp_num} — YOLO11s | {exp['name']}")
    print("=" * 70)
    ...
    lines = [
        "=" * 70,
        f"  EXPERIMENT {exp_num} — YOLO11s on {exp['name']}",
        "=" * 70,
        "",
        "  SETUP",
        f"    Model            : YOLO11s ({WEIGHTS.name})",
        f"    Dataset          : {exp['name']}",
...
def print_summary(results: list[dict]) -> None:
    ...
    summary_path = HOME / "FYP/results/summary/yolov11_experiments_summary.txt"
...
def main() -> int:
    parser = argparse.ArgumentParser(
        description="YOLO11s on all 5 construction safety datasets",
  ...
  python3 train_yolov11_all.py --datasets sh17 sfchd
    ...
    if not WEIGHTS.exists():
        ...
        if src.exists():
            ...
            shutil.move(str(src), str(WEIGHTS))
            print(f"  Saved weights → {WEIGHTS}")
        else:
            print("  ERROR: auto-download failed. Run manually:")
            print("    python3 -c \"from ultralytics import YOLO; YOLO('yolo11s.pt')\"")
            print(f"  then move yolo11s.pt to {WEIGHTS}")
            return 1
